...
def chat(request: ChatRequest):
    ...
    try:
        ...
        if not conversation_id:
            try:
                ...
                conversation_id = conversation_service.create_conversation(
                    title=title,
                    metadata={"tone": request.tone.value if request.tone else "conversational"},
                )
        ...
        # Save the user message if we have a conversation_id
        if conversation_id:
            try:
        ...
        answer, sources, follow_up_questions = rag_engine.answer_question(
            request.message, tone=request.tone.value if request.tone else "conversational"
        )
        ...
        for i, doc in enumerate(sources):
            ...
            if len(content) > excerpt_length:
                ...
                context_after = (
                    content[excerpt_length : excerpt_length + 100]
                    if len(content) > excerpt_length
                    else None
                )
            else:
        ...
        # Save the assistant response if we have a conversation_id
        if conversation_id:
            try:
                conversation_service.add_message(
                    conversation_id=conversation_id,
                    ...
                    content=answer,
                    sources=[
                        {"page": doc.page, "content": doc.content, "score": doc.score}
