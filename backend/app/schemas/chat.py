...
class ChatRequest(BaseModel):
    ...
    conversation_id: Optional[int] = Field(default=None, description="Optional conversation ID to save messages")
    tone: Optional[ResponseTone] = Field(default=ResponseTone.CONVERSATIONAL, description="Response tone/style")


class ChatResponse(BaseModel):
    """Chat response model with sources."""
    answer: str = Field(..., description="AI-generated answer")
    sources: List[SourceDocument] = Field(default_factory=list, description="Source documents with page references")
